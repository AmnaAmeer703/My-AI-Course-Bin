...
class ModelEvaluation:
    ...
    def get_best_model(self) -> Optional[Proj3Estimator]:
        
        try:
            bucket_name = self.model_eval_config.bucket_name
            model_path=self.model_eval_config.s3_model_key_path
            proj3_estimator = Proj3Estimator(bucket_name=bucket_name,
                                               model_path=model_path)

            if proj3_estimator.is_model_present(model_path=model_path):
                return proj3_estimator
            return None
        except Exception as e:
