...
X['Text'] = X['Text'].apply(remove_punctuation_func)

X['Text'] = list(X['Text'])

df[df['Score'] == 'Negative' ]


from wordcloud import WordCloud
wc = WordCloud(width=500,height=500,min_font_size=10,background_color='black')
spam_wc = wc.generate(df[df['rating_score']=='Negative']['Text'].str.cat(sep=""))
plt.imshow(spam_wc)

from sklearn.preprocessing import LabelEncoder
le = LabelEncoder()
df['raring_score'] = le.fit_transform(df['rating_score'])

label_map = dict(zip(le.classes_, le.transform(le.classes_)))

print(label_map)
print(df['rating_score'].value_counts())
print(df['rating_score'].unique())

y = df['raring_score'].values
...
target_names = ['Postive','Neutral','Negtive']
