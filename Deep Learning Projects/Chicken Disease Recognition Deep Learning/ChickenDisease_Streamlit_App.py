...
st.header("Chicken Disease Recognition App")
st.caption("Upload an image. ")
st.caption("The application will infer the one label out of 4 labels: Coccidiosis,Healthy,New Castle Disease,Salmonella")
st.caption("Warning: Do not click Recognize button before uploading image. It will result in error.")

# Load the model
model = load_model("model.h5")
# Define the class names
class_name = {'Coccidiosis':0,'Healthy':1,'New Castle Disease':2,'Salmonella':3}
...
def predict_label(image2):
    ...
    predicted_label = {index_to_label[np.argmax(class_index)]}
