...
st.header("Pneumonia Chest Xray Recognition App")
st.caption("Upload an image. ")
st.caption("The application will infer the one label out of 2 labels: Pneumonia,Normal")
st.caption("Warning: Do not click Recognize button before uploading image. It will result in error.")

# Load the model
model = load_model("Chest_Xray.h5")
# Define the class names
class_name = {'Normal':0,'PNEUMONIA':1}
...
def predict_label(image2):
    ...
    class_index = np.argmax(pred_labels)
    index_to_label = {v: k for k, v in class_name.items()}
    # Get the predicted class label
    predicted_label = {index_to_label[np.argmax(pred_labels)]}
