...
tracking.set_experiment("MLflow Quickstart5")

df = pd.read_csv('Section1-Question1-InsurranceData.csv')

# Enable autologging for scikit-learn
tracking.sklearn.autolog()
...
with tracking.start_run():
    ...
    # Log the model
    model_info = tracking.sklearn.log_model(sk_model=pipe)
    ...
    tracking.log_metric("accuracy", accuracy)

    tracking.set_tag("Training Info -", "Basic Random Forest model for data")
    ...
    loaded_model = tracking.pyfunc.load_model(model_info.model_uri)
...
tracking.set_experiment("Hyperparameter Tuning Experiment")
def objective(trial):
    # Setting nested=True will create a child run under the parent run.
    with tracking.start_run(nested=True, run_name=f"trial_{trial.number}") as child_run:
        ...
        tracking.log_metrics({"error": error})

        # Log the model file
        tracking.sklearn.log_model(pipe,'Random-Forest-Model')
        # Make it easy to retrieve the best-performing child run later
        trial.set_user_attr("run_id", child_run.info.run_id)
        return error
    
    # Create a parent run that contains all child runs for different trials
with tracking.start_run(run_name="study") as run:
    # Log the experiment settings
    n_trials = 30
    tracking.log_param("n_trials", n_trials)

    study = optuna.create_study(direction="minimize")
    study.optimize(objective, n_trials=n_trials)

    # Log the best trial and its run ID
    tracking.log_params(study.best_trial.params)
    tracking.log_metrics({"best_error": study.best_value})
    if best_run_id := study.best_trial.user_attrs.get("run_id"):
        tracking.log_param("best_child_run_id", best_run_id)
...
def monitor_dataset_quality(dataset, reference_dataset=None):
    ...
    with tracking.start_run(run_name="Data_Quality_Check"):
        tracking.log_input(dataset, context="quality_monitoring")
        tracking.log_metrics(quality_metrics)
        ...
        if reference_dataset is not None:
            ...
            tracking.log_metrics(drift_metrics)
