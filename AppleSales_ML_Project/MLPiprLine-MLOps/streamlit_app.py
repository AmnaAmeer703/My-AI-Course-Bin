...
'AirPods Max','iPhone 12 mini','iPad mini (5th Generation)','MacBook (Retina)','AirTag','MacBook Pro 16-inch','iPhone 13','Apple Watch Series 6','iMac 24-inch','HomePod mini','iMac with Retina Display','iPhone 13 Pro Max','iPad Pro 12.9-inch','Magic Trackpad','Leather Case for iPhone','HomePod','AirPods (2nd Generation)','iCloud','Smart Cover for iPad','Apple Watch Series 9','Apple Watch Series 7','Lightning to USB Cable','Mac Mini','iPad mini (6th Generation)','iMac 27-inch','iPad Pro (M1)','Silicone Case for iPhone','Apple Watch Ultra','iPad (10th Generation)','AirPods Pro','iPhone 12 Pro Max','HomePod (2nd Generation)',
'Apple TV (3rd Generation)','Apple News+','Beats Fit Pro','iPad Pro 11-inch','MagSafe Charger','Apple Music','Magic Mouse',
'Mac Pro (Rack)','MagSafe Battery Pack','MacBook','iPhone 13 mini','MacBook Air (Retina)','iPhone 14','Beats Fit Pro','iPad Pro 11-inch']
...
st.title('APPLE RETAIL SALES PREDICTIONS')


Product_Name = st.selectbox('select the Product Name',sorted(Product_Name))
...
with col4:
    Store_Name = st.selectbox('select the Store Name',sorted(Store_Name))
with col5:
    City = st.selectbox('select the City',sorted(City))

Country = st.selectbox('select the Country',sorted(Country))

category_name = st.selectbox('select the category name',sorted(category_name))

col8, col9 = st.columns(2)
with col8:
    sale_year = st.number_input('sale_year')
with col9:
    sale_month = st.number_input('sale_month')

col10, col11 = st.columns(2)
with col10:
    Launch_Year = st.number_input('Launch_Year')
with col11:
    Launch_Month = st.number_input('Launch_Month')

if st.button('APPLE RETAIL SALES Predictibility'):
