...
mlflow.log_artifact("pipe.pkl")

with mlflow.start_run():
    pipe.fit(X_train,y_train)
    # Log the model
    model_info = mlflow.sklearn.log_model(python_model=pipe,
...
def objective(trial):
    ...
    with mlflow.start_run(nested=True, run_name=f"trial_{trial.number}") as child_run:
        rf_max_depth = trial.suggest_int("rf_max_depth", 2,10,32)
        rf_n_estimators = trial.suggest_int("rf_n_estimators",30, 50, 300, step=10)
        ...
        # Log the model file
        mlflow.sklearn.log_model(regressor_obj, artifact_path='pipe')
